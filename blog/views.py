...
def track_user(request):
    response = render(request, 'track_user.html') # store the response in response variable
...
def lousy_login(request):
    ...
    return render(request, 'lousy_login.html')


def lousy_secret(request):
    if not request.session.get('logged_in'):
        return redirect('lousy_login')
    return render(request, 'lousy_secret_page.html')
